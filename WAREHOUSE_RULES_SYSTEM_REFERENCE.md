...
##### **🔥 CRITICAL ANOMALY COUNTING BEHAVIOR**
The overcapacity rule uses **location differentiation logic**:

**Storage Locations** (`backend/src/rule_engine.py:2002-2014`):
```python
...
for location, info in storage_overcapacity.items():
    ...
    for _, pallet in pallets_in_loc.iterrows():  # ← ALL PALLETS FLAGGED
        anomalies.append({
            'anomaly_type': 'Storage Overcapacity',
            'priority': 'VERY_HIGH',  # CRITICAL
            'business_context': 'CRITICAL - Data integrity requires individual pallet investigation'
        })
```

**Special Locations** (`backend/src/rule_engine.py:2016-2042`):
```python
...
anomalies.append({
    ...
    'business_context': 'WARNING - Space management focus'
})
```

**Anomaly Count**:
- **Storage locations**: **ALL pallets** in overcapacity location flagged (not just excess)
- **Special locations**: 1 representative alert per overcapacity location
- **Example**: Location with 10 pallets (capacity=1) → **10 anomalies** for storage, **1 anomaly** for special

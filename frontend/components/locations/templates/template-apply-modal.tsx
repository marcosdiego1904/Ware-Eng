...
interface TemplateApplyModalProps {
  ...
  template: {
    ...
    staging_areas?: any[];
    dock_areas?: any[];
    receiving_areas_template?: any[];
    staging_areas_template?: any[];
    dock_areas_template?: any[];
  } | null;
  ...
  applyResult?: {
    ...
    virtual_location_summary?: {
      storage_locations?: number;
      special_areas?: number;
      total_possible_locations?: number;
    };
  } | null;
}
...
export function TemplateApplyModal({
  ...
  const calculateTotals = () => {
    ...
    // Calculate special areas: explicit areas + auto-generated aisle areas
    // Handle both possible property names (receiving_areas vs receiving_areas_template)
    const receivingAreas = (template as any).receiving_areas_template || template.receiving_areas || [];
    const stagingAreas = (template as any).staging_areas_template || template.staging_areas || [];
    const dockAreas = (template as any).dock_areas_template || template.dock_areas || [];
    
    const explicitSpecialAreas = receivingAreas.length + stagingAreas.length + dockAreas.length;
    
    // The system automatically creates one special area per aisle
    const autoGeneratedAisleAreas = template.num_aisles;
    
    const specialAreas = explicitSpecialAreas + autoGeneratedAisleAreas;
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  ...
                  <div>
                    <div className="text-2xl font-bold text-green-600">
                      {(applyResult.virtual_location_summary?.storage_locations || applyResult.storage_locations || totals.storageLocations).toLocaleString()}
                    </div>

...
class VirtualLocationCompatibilityManager:
    ...
    def _get_all_virtual_locations(self, virtual_engine, limit: int = 1000, created_by: int = None) -> List[Dict[str, Any]]:
        ...
        # FIXED: Merge physical and virtual special areas instead of skipping
        # Physical special areas (user-created) take priority, then add virtual ones that don't conflict
        print(f"[VIRTUAL_COMPAT] Found {len(physical_special_areas)} physical special areas - merging with virtual")

        # Add virtual special areas that don't conflict with physical ones
